...
class MyVisitsPage extends Component
{
    ...
    public function mount(): void
    {
        ...
        abort_unless($user instanceof User && $user->canAny(['ViewOwn:Visit', 'EditOwn:Visit', 'Create:Visit']), 403);
    ...
    private function visits(): array
    {
        ...
        if ($search !== '') {
            $this->enforceSearchRateLimit('my-visits-page');
        }
    ...
    private function mapParticipant(Visit $visit, Visitor $visitor): array
    {
        ...
        return [
            ...
            'can_print_badge' => false,
            'can_check_in' => false,
            'check_in_label' => __('Check-in'),
            'can_check_out' => false,
            'badge_url' => null,
        ];
    }
    ...
    private function resolveParticipantStatus(string $visitStatus, object $pivot): array
    {
        if (filled($pivot->checked_out_at)) {
            return ['label' => __('Ausgecheckt'), 'class' => 'text-base-content/60'];
        }

        if (filled($pivot->checked_in_at)) {
            return ['label' => __('Eingecheckt'), 'class' => 'text-success'];
        }

        if (filled($pivot->badge_printed_at)) {
            return ['label' => __('Ausweis bereit'), 'class' => 'text-success'];
        }

        if ($visitStatus === VisitStatusEnum::Draft->value) {
            return ['label' => __('Entwurf'), 'class' => 'text-base-content/60'];
        }

        if ($visitStatus === VisitStatusEnum::Canceled->value) {
            return ['label' => __('Abgesagt'), 'class' => 'text-error'];
        }

        return ['label' => __('Geplant'), 'class' => 'text-warning'];
    }
